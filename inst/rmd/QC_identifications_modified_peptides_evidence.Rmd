...
```{r id-mod-peptides-evidence, eval=TRUE, warning=FALSE, echo=FALSE, message=FALSE, error=FALSE}
dt <- copy(evidence)
...
p <- ggplot(dt, aes(x = as.factor(Run), y = N, color = condition, label=`raw file`)) +
  ...
  ggtitle("Modified Peptides - Number of identifications from evidence.txt")
