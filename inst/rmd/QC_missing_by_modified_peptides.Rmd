...
```{r missing-mod-peptides, eval=TRUE, warning=FALSE, echo=FALSE, message=FALSE, error=FALSE}
...
p <- ggplot(dt, aes(x = ReplicatePC)) +
  ...
  scale_x_continuous("Percentage of measurements per mod peptide", labels = scales::percent, limits = c(0, 1.15), breaks = seq(0, 1, 0.1)) +
  annotate('text', x = 0.5, y = 0.8*ymax, label=str_c("Number of mod peptides with\nmissing values <= 30%:\n ", dt[ReplicatePC >= 0.7, .N])) +
  ggtitle("Modified peptide measurements availability")
