...
```{r sample-de-peptides, eval=TRUE, warning=FALSE, echo=FALSE, message=FALSE, error=FALSE}
    ...
    if (class(test) != "try-error"){
      ...
      if (num_files > 10) {
        corrplot(DT_corMatrix, type = "upper", tl.cex = 0.5, mar = c(1,0,1.5,0),
                 title = "Correlation Matrix - DE peptides")
        
      } else {
        corrplot(DT_corMatrix, type = "upper", tl.cex = 0.5, mar = c(1,0,1.5,0), addCoef.col = "white", number.cex = .5,
                 title = "Correlation Matrix - DE peptides")
