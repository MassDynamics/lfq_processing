...
```{r id-mod-peptides, eval=TRUE, warning=FALSE, echo=FALSE, message=FALSE, error=FALSE}
...
p <- ggplot(dt, aes(x = as.factor(Run), y = N, color = condition, label=Replicate)) +
  ...
  scale_y_continuous("# Modified Peptides") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.2),
        panel.grid.major.y = element_blank(),
        panel.border = element_blank(),
        axis.ticks.y = element_blank()
        ) +
  ggtitle("Modified Peptides - Number of identifications from modificationSpecificPeptides.txt")
