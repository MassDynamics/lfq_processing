...
```{r id-proteins-evidence, eval=TRUE, warning=FALSE, echo=FALSE, message=FALSE, error=FALSE}
dt <- copy(evidence)
...
p <- ggplot(dt, aes(x = as.factor(Run), y = N, color = condition, label=`raw file`)) +
  ...
  scale_y_continuous("# Proteins") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.2),
        panel.grid.major.y = element_blank(),
        panel.border = element_blank(),
        axis.ticks.y = element_blank()
        ) +
  ggtitle("Proteins - Number of identifications from evidence.txt")
